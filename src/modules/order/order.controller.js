...
export const cancelOrder =  async (req, res, next)  => {
    ...
    const order = Order[orderIndex];

    // Check if the order is older than 1 day
    const oneDayInMillis = 24 * 60 * 60 * 1000;
    const currentTime = new Date();
    const orderTime = new Date(order.createdAt);

    if ((currentTime - orderTime) > oneDayInMillis) {
        return res.status(400).json({ message: 'Order cannot be canceled after 1 day' });
    }
